...
def train_and_pre(mode, zu_name):
    # -----------------load data----------------#
    np.random.seed(1)
    dna1 = np.expand_dims(pickle.load(open('../Data/encode_' + mode + '/dna1_B73' + zu_name + '_500_' + mode, 'rb')), 3)
    dna2 = np.expand_dims(pickle.load(open('../Data/encode_' + mode + '/dna2_B73' + zu_name + '_500_' + mode, 'rb')), 3)
    print(dna1.shape)

    file = pd.read_csv('../Data/exp_int_data_' + mode + '/B73(' + zu_name + ')values_500_' + mode + '.csv',
                       encoding='gbk')
    ann1 = file['Annotation1'].values.astype('str')
    ann2 = file['Annotation2'].values.astype('str')
    exp = file['express'].values.astype('float')
    ...
    slide1 = int(len(ann1) * 0.6)
    slide2 = int(len(ann1) * 0.8)
    ...
    ckpt = keras.callbacks.ModelCheckpoint('../Model/' + mode + '/' +mode+'_'+ zu_name + '.h5',
                                           monitor='val_pearson',
                                           verbose=1,
                                           save_best_only=True,
    ...
    history = model.fit([dna1[:slide1], dna2[:slide1]],
                        ...
                        callbacks=[ckpt])
    if mode == 'dou':
        show_train_history(history.history, s=zu_name + '_data_augment', locate='../Result/pic/' + mode + '/')
    else:
    ...
    array = np.array([ann1[slide2:],ann2[slide2:],test_label, rslt[:, 0]]).T
    df = pd.DataFrame(array, columns=['Annotation1','Annotation2','exp', 'pre'])
    df.to_csv('../Result/' + mode + '/' + mode + '_' + zu_name + '.csv', index=False)
